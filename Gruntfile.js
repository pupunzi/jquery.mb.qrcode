...
module.exports = function (grunt) {
	...
	grunt.initConfig({
		...
		copy: {
			dist: {
				files: [
					{flatten: true, expand: true, cwd: 'vendor/jquery-qrcode-master/', src: ['jquery.qrcode.min.js'], dest: 'src/dep/'},
		...
		includereplace: {
			dist: {
				...
				files  : [
					...
					{src: 'dist/*.css', expand: true}
				]
			}
		},

		jsbeautifier: {
			files  : ['src/*.js', '!dist/*.min.js', 'src/*.html', 'src/*.tmpl', 'src/*.css'],
			options: {
				html: {
					braceStyle         : "collapse",
					indentChar         : "\t",
		...
		bump: {
			options: {
				...
				push              : true,
				pushTo            : 'https://github.com/pupunzi/jquery.mb.qrcode.git',
		...
		watch: {
			files: ['src/*.css', 'src/*.js', 'src/*.html', 'Gruntfile.js'],
			tasks: ['default']
		}
